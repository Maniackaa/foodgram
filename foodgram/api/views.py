...
from api.serializers import (ChangePassSerializer, FavoriteSerializer,
                             FollowSerializer, IngredientSerializer,
                             RecipeSerilizer, RecipeWriteSerializer,
...
class UsersViewSet(mixins.CreateModelMixin,
    ...
    def change_password(self, request):
        ...
        if serializer.is_valid(raise_exception=True):
            return Response({'Result': 'Пароль успешно изменен'},
    ...
    def subscribe(self, request, *args, **kwargs):
        ...
        if request.method == 'POST':
            ...
            if serializer.is_valid(raise_exception=True):
                ...
                return Response({'Подписка!': serializer.data},
...
class RecipeViewSet(viewsets.ModelViewSet):
    ...
    def shopping_cart(self, request, **kwargs):
        ...
        user = self.request.user
        if request.method == 'POST':
            if ShopCart.objects.filter(author=user,
                                       recipe=recipe).exists():
            ...
            return Response(serializer.errors,
                            status=status.HTTP_400_BAD_REQUEST)
        # Если DELETE
        if not ShopCart.objects.filter(author=user,
                                       recipe=recipe).exists():
            return Response({'errors': 'Рецепт не найден'},
                            status=status.HTTP_404_NOT_FOUND)
        ShopCart.objects.get(recipe=recipe).delete()
        return Response('Рецепт удалён из корзины.',
                        status=status.HTTP_204_NO_CONTENT)
