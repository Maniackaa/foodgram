...
from app.models import (Favorite, Follow, Ingredient, IngredientInRecipe,
                        Recipe, ShopCart, Tag)
...
class IngredientInRecipeSerializer(serializers.ModelSerializer):
    ...
    #     write_only=True, queryset=Ingredient.objects.all())
...
class RecipeSerilizer(serializers.ModelSerializer):
    ...
    ingredients = IngredientInRecipeSerializer(many=True, source='components',
                                               read_only=True)
...
class RecipeWriteSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Recipe
        fields = (
            'id', 'ingredients', 'tags', 'name', 'text', 'cooking_time',
            'author', 'image',
        )
    ...
    def to_representation(self, instance):
        ...
        ingredients['ingredients'] = IngredientInRecipeSerializer(
            instance.components.all(), many=True).data
    ...
    def create(self, validated_data, *args, **kwargs):
        ...
        for ingredient_in_recipe in ingredients:
            ...
            obj, res = IngredientInRecipe.objects.get_or_create(
                recipe=recipe,
    ...
    def update(self, instance: Recipe, validated_data):
        ...
        for ingredient_in_recipe in ingredients:
            ingredient = ingredient_in_recipe['id']
            amount = ingredient_in_recipe['amount']
            obj, res = IngredientInRecipe.objects.update_or_create(
                recipe=instance,
                ingredient=ingredient,
                amount=amount)
...
class ShopCartSerializer(serializers.ModelSerializer):
    ...
    image = serializers.ImageField(source='recipe.image',
                                   read_only=True)
    coocking_time = serializers.IntegerField(source='recipe.cooking_time',
                                             read_only=True)
...
class FollowSerializer(serializers.ModelSerializer):
    ...
    def get_is_subscribed(self, follow):
        user = self.context.get('request').user
        if user.is_authenticated:
            return Follow.objects.filter(user=follow.user,
    ...
    def validate(self, attrs):
        ...
        if author == user:
            raise ValidationError('Не положено!',
                                  code=status.HTTP_400_BAD_REQUEST)
        return attrs
